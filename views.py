...
def home(request):
    if not request.user.is_authenticated:
        demo_videos = VideoPost.objects.all().filter(status=1).order_by('-id')[:5]
        params = {'videos': demo_videos}
        return render(request, 'welcome.html', params)
    else:
        all_videos = VideoPost.objects.all().filter(status=1).order_by('-id')
...
def upload_video(request):
    if request.method == 'POST':
        ...
        upload_video = VideoPost(user=user_obj, title=title, desc=desc, video_file=video_file, thumbnail=thumb_nail, category=cate, status=0)
        upload_video.save()
...
def watch_video(request, video_id):
    ...
    try:
        ...
        messages.warning(request, 'Вы не зарегистрированы, но все равно можете просматривать видео.')
    ...
    try:
        ...
        messages.warning(request, 'Вы не зарегистрированы, ваши просмотры и лайки не будут засчитываться.')
...
def profile(request, session_username):
    ...
    video_cat_sport = VideoPost.objects.filter(user=session_obj, category='Sport').order_by('-id')
    video_cat_education = VideoPost.objects.filter(user=session_obj, category='Education').order_by('-id')
    video_cat_other = VideoPost.objects.filter(user=session_obj, category='Other').order_by('-id')
    ...
    if request.user in profile_data.subscribers.all():
        ...
        subscribed = False
    params = {'subscribed':subscribed,'session_obj':session_obj,'user_data':profile_data, 'videos': user_posts, 'sci': video_cat_science, 'blogs': video_cat_blogs, 'sport': video_cat_sport, 'edu':video_cat_education, 'other': video_cat_other}
